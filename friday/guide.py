...
CHAPTERS = [
    ...
    ("Защитник", "shield", [
        ("1. Проверьте защиту", "Откройте Защитник → Антивирус. Пятница показывает статус Microsoft Defender, запускает быструю проверку и открывает «Безопасность Windows». Это не заменяет встроенные уведомления Defender.", "security"),
        ("2. Включите присмотр", "Защитник → Присмотр → глаз. Локально записываются запуск и закрытие приложений; при включённом флажке создаются снимки экрана с выбранным интервалом. Кнопка документа создаёт отчёт в data/security. Наблюдение всегда отображается в интерфейсе и его можно выключить.", "security"),
        ("3. Охраняйте сессию", "«Пятница, охраняй ПК» открывает охрану и запрашивает подтверждение блокировки Windows. Для возврата нужен пароль Windows. Перетащите важные документы или ярлыки в список правил — они появятся в локальном отчёте.", "security"),
        ("4. Родительский контроль", "Добавьте разрешённые приложения перетаскиванием. Режим отмечает запуск других приложений в локальном журнале. Для строгого разделения доступа используйте отдельную учётную запись Windows и блокировку сессии.", "security")]),
    ("Оформление", "palette", [
