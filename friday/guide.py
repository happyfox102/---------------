...
CHAPTERS = [
    ...
    ("Документы и диктовка", "file", [
        ...
        ("2. Работайте с таблицей", "«Запиши 1500 в B3», «прочитай B3», «копируй B3 в C3», «сделай сумму из A2:A20 в B2», «рассчитай столбец D как сумму столбцов A и B», «создай линейную диаграмму из A1:B20», «отмени изменение». Формулы, заполнение столбца и диаграммы сохраняются в выбранной книге. Кнопка таблицы выбирает книгу и её активный лист.", "excel"),
    ...
    ("Защитник", "shield", [
        ("1. Проверьте защиту", "Настройки → Защитник → Антивирус. Статус и сканирование Defender запускаются кнопками в фоне. При запуске приложения проверка не выполняется. Главная вкладка Защитника содержит только журнал времени, событий и результатов.", "security"),
        ("2. Включите присмотр", "Настройки → Защитник → Присмотр → глаз. Пока режим включён, каждые 10 секунд проверяются изменения процессов, при выборе снимков сохраняется экран с заданным интервалом. Уже работающие программы не записываются как новые. Короткие процессы между опросами могут быть пропущены. Журнал продолжает работать при свёрнутом окне; выключенный режим не опрашивает процессы.", "security"),
        ("3. Охрана и отчётность", "«Пятница, охраняй ПК» запрашивает блокировку Windows. Журнал различает принятый Windows запрос, ошибку и отмену. Список файлов является памяткой, а не файловыми правами. Пятница не видит попытки запуска в заблокированной сессии и не выдаёт наблюдение за успешную блокировку программы. Локальный отчёт доступен в настройках присмотра.", "security"),
        ("4. Детский браузер", "Настройки → Защитник → Родительский контроль: установите PIN, заполните разрешённые и запрещённые домены, задайте часы доступа, сохраните правила и включите контроль. Кнопка глобуса открывает отдельный браузер. Только одобренные HTTPS-домены доступны; неизвестные сайты, новые окна, загрузки файлов и доступ вне расписания блокируются. Для поиска добавьте bing.com; поиск из адресной строки использует строгий режим. Отдельные ресурсы сайта могут требовать добавления доверенных доменов. Правила не действуют в Chrome/Edge вне Пятницы; для запрета обхода нужна отдельная ограниченная детская учётная запись Windows.", "security"),
        ("5. Оценка запросов ИИ", "Флажок оценки ИИ отправляет поисковый запрос из детского браузера выбранному провайдеру, не чаще раза в минуту. В отчёт и уведомление поступает оценка риска мошенничества, контента 18+ или призывов к насилию. Это предположение, не проверенная репутация сайта. Неизвестные сайты остаются закрыты даже при ошибке ИИ. Пароли и страницы личных браузеров не собираются. PIN защищает обычный доступ к настройкам и выходу; администратор Windows может обойти защиту приложения.", "security")]),
    ("Оформление", "palette", [
